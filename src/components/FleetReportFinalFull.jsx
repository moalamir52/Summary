                  ...
                  <thead>
                    ...
                    <tr style={{ background: '#f5f5f5' }}>
                      ...
                      <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f5f5f5' }}>
                        ...
                        <td style={{ padding: '10px 8px', borderBottom: '1px solid #ffe082', textAlign: 'center' }}>{colorizeInvygoYelo(car.Model)}</td>
                    ...
                    <tbody>
                      {applyTableFilters(filtered).map((car, i) => (
                        <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#f5f5f5', textAlign: 'center' }}>
                          ...
                          <td style={{ padding: '6px 2px', borderBottom: '1px solid #ffe082', textAlign: 'center', color: '#7b1fa2', fontWeight: 'bold' }}>{colorizeInvygoYelo(car.Model)}</td>
...
function getCarsExpiringInMonth(month, year) {
  // month: رقم الشهر (1-12)، year: السنة المطلوبة
  return data.filter(car => {
    const exp = car.RegExp || car["Reg Exp"];
    if (!exp) return false;
    // دعم تواريخ بصيغة yyyy-mm-dd أو dd/mm/yyyy
    let d;
    if (typeof exp === "string" && exp.includes("-")) d = new Date(exp);
    else if (typeof exp === "string" && exp.includes("/")) {
      const [day, mon, yr] = exp.split("/");
...
// دالة تلوين النص تلقائياً في كل مكان
function colorizeInvygoYelo(text) {
  if (!text) return '';
